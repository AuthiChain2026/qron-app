#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Register the QRON Stripe webhook endpoint AND set Vercel env vars.
#
# Prerequisites:
...
#
# What it does:
#   1. Creates (or finds) a Stripe webhook for https://qron.space/api/webhook
#   2. Sets STRIPE_SECRET_KEY + STRIPE_WEBHOOK_SECRET in Vercel qron-app project
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

WEBHOOK_URL="${QRON_URL:-https://qron.space}/api/webhook"
VERCEL_PROJECT_ID="prj_jkFURGmAhmo0sUSZjNpRlc01SRL3"
...
if [[ -n "$FOUND_ID" ]]; then
  ...
  curl -s -X POST "https://api.stripe.com/v1/webhook_endpoints/${FOUND_ID}" \
    ...
    -d "enabled_events[2]=customer.subscription.deleted" > /dev/null
  echo "  Updated."
  echo
  ...
  echo "  Or set STRIPE_WEBHOOK_SECRET manually below."
  ...
  RESULT=$(curl -s -X POST "https://api.stripe.com/v1/webhook_endpoints" \
    ...
    -d "description=QRON app — sales fulfillment, subscription lifecycle")
...
set_vercel_env() {
  ...
  # Try to delete existing env var first (ignore errors)
...
echo
echo "Setting Vercel environment variables for qron-app..."
...
echo
echo "Done! Redeploy qron-app for changes to take effect."
