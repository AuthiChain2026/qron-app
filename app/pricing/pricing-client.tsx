...
export function CheckoutModal({ plan, label, price }: { plan: string, label: string, price: string }) {
  ...
  return (
    <>
      {/* Button always renders — SSR safe */}
      <button
        onClick={() => setOpen(true)}
        style={{ width:'100%', background:'#c9a227', color:'#000', border:'none', borderRadius:10, padding:'13px', fontSize:15, fontWeight:700, cursor:'pointer' }}
      >
        Get {label} →
      </button>

      {/* Modal only renders client-side after mount */}
      {mounted && open && (
        <div
        ...
        >
          <div style={{ background:'#111', border:'1px solid rgba(201,162,39,.3)', borderRadius:16, padding:32, maxWidth:400, width:'100%', margin:'0 24px' }}>
            ...
            <button
              ...
              style={{ width:'100%', background:'#c9a227', color:'#000', border:'none', borderRadius:8, padding:'12px', fontSize:15, fontWeight:700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading || !email.includes('@') ? 0.6 : 1 }}
            >
              {loading ? 'Redirecting...' : `Continue to ${price} →`}
            </button>
            <button
              onClick={() => setOpen(false)}
              style={{ width:'100%', background:'transparent', color:'#666', border:'none', padding:'10px', cursor:'pointer', marginTop:8, fontSize:13 }}
            >
              Cancel
            </button>
          </div>
